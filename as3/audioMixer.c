...
static unsigned long playbackBufferSize = 2205;
...
playbackSound_t freeValue = {NULL, 0};
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	for(int i = 0; i < MAX_SOUND_BITES; i++){
		if(soundBites[i].pSound == NULL){
			playbackSound_t newSound = {pSound, 0};
			soundBites[i] = newSound;
			i = MAX_SOUND_BITES;
			found = 0;
		}
	}
	...
	if(found == 1){
		//printf("Queue Full unable to queue sound");
	}
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	...
	for(int i = 0; i < MAX_SOUND_BITES; i++){
		...
		if(sound.pSound != NULL){
			short* soundData = sound.pSound->pData;
			int samples = sound.pSound->numSamples;
			int location = sound.location;
			for(int j = 0; j < size; j++){
				int soundBite = playbackBuffer[j];
				soundBite = soundBite + soundData[location];
				soundBite = fmin(SHRT_MAX, soundBite);
				soundBite = fmax(SHRT_MIN, soundBite);
				playbackBuffer[j] = (short)soundBite;
				location++;
				if(location >= samples){
					soundBites[i] = freeValue;
					j = size;
